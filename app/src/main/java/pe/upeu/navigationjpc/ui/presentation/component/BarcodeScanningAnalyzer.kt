...
fun CameraView(
    ...
    AndroidView(
        ...
        factory = { ctx ->
            // Asegúrate de inicializar correctamente PreviewView
            val previewView = PreviewView(ctx)
            cameraProviderFuture.addListener({
                val preview = Preview.Builder().build()
                // Usa el surfaceProvider de PreviewView
                ...
                cameraProvider.bindToLifecycle(
                    ...
                    cameraSelector,
                    preview,
                    imageAnalysis // Vincula los casos de uso a la cámara
                )
            }, executor)

            previewView // Devolver PreviewView
        }
    )
}